...
#' @param max UNUSED - not supported by \code{botor}; all objects will be listed
#' @export
...
list_files_in_buckets <- function(bucket_filter=NULL, prefix=NULL, path_only=FALSE, max=NULL) {
  uri <- httr::parse_url(full_s3_path(bucket_filter))
  uri$path <- prefix
  uri <- httr::build_url(uri)

  tryCatch(
    {
      objects <- botor::s3_ls(uri) %>%
        dplyr::mutate(
          ...
          # Supported by s3tools but not botor
          etag = NA,
          storageclass = NA
        ) %>%
        dplyr::select(
    ...
    error = function(c) {
      message("Can't list files in ", bucket_filter, " with prefix ", prefix)
